...
/// MNS (Modèle Numérique de Surface) issu du LiDAR HD IGN.
pub struct Mns {
...
impl Mns {
    pub fn new(output_path: Option<String>) -> Result<Self> {
        ...
        let path_save_tiff = output_path_buf.join("mns_lidar_hd.tif");
        let path_temp_tiff = PathBuf::from(TEMP_PATH).join("mns.tiff");
    ...
    pub fn run_internal(&mut self) -> Result<()> {
        self.ign_collect.execute_ign("mns")?;

        if !self.path_temp_tiff.exists() {
            anyhow::bail!(
                "MNS file not found at {:?}. Make sure execute_ign('mns') was called successfully.",
                self.path_temp_tiff
            );
        }
    ...
    fn copy_to_output(&self) -> Result<()> {
        ...
        std::fs::copy(&self.path_temp_tiff, &self.path_save_tiff).context(format!(
            "Failed to copy MNS from {:?} to {:?}",
            self.path_temp_tiff, self.path_save_tiff
        ))?;

        println!("MNS saved to: {:?}", self.path_save_tiff);
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn test_mns_new() {
        let mns = Mns::new(None).unwrap();
        assert!(mns
            .path_save_tiff
            .to_string_lossy()
            .contains("mns_lidar_hd.tif"));
    }

    #[test]
    fn test_mns_set_bbox() {
        let mut mns = Mns::new(None).unwrap();
        mns.set_bbox(-1.152704, 46.181627, -1.139893, 46.18699);
        assert!(mns.bbox.is_some());
    }
    ...
    fn test_mns_set_crs() {
        let mut mns = Mns::new(None).unwrap();
        mns.set_crs(2154);
        assert_eq!(mns.geo_core.get_epsg(), 2154);
    }
}
